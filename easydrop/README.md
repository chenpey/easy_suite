...
点击“图片与文件”上传区、拖放文件，或在主页面未打开弹窗时直接粘贴图片/文件，可一次添加多个文件并立即开始上传。粘贴截图等没有文件名的图片时会自动生成带正确扩展名的文件名；即使文本框已有焦点，剪贴板中的图片/文件也会优先进入上传队列。默认同时处理 3 个文件；每个非空文件再使用 R2 Multipart Upload 并发上传分片。文件级和分片级并发都受服务端配置控制。
