...
- **设置面板英文界面与窄屏布局优化**：优化账户设置面板在英文模式下的布局与文本对齐，调整标签与控件宽度，避免英文字符较长时的折行与错位问题；改进移动端/窄屏下的弹窗间距与排版细节。

## 1.2.5 更新

- **国际化架构重构**：所有文案收口到独立字典（`web/i18n.js`），支持 `t(key, ...args)` 参数插值。
- **静态与动态解耦**：HTML 静态文案采用 `data-i18n*` 标记并一次性渲染，JS 动态文案统一调用 `t(...)` 获取，彻底移除 `MutationObserver` 文本模糊替换。
- **自动化 Key 对齐检查**：增加 `scripts/check-i18n.mjs`，接入 `check` 与 `test` 流水线，缺少翻译或未对齐直接阻断。

## 1.2.4 更新

- **语言选择规范化**：设置面板语言选择器保持「中文」与「English」原样显示，标签统一为「语言/Language」，避免二次翻译造成混淆。
