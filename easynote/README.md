...
- **设置面板数据操作排版优化**：将原本堆叠在单行的备份与导入按钮拆分为独立的「导出」与「导入」行，左右对称、单行对齐，彻底解决英文模式下按钮意外折行（3+1 结构）与留白问题；移动端自动均分 50% 宽度。
- **侧边栏收起交互与视觉优化**：展开侧栏按钮图标由歧义的三个点替换为语义明确的 `<PanelLeftOpen />`，与收起图标完全配对；新增右侧外边距与标题拉开舒适间距。
- **左侧提示框截断修复**：为侧边栏展开按钮与列表首项按钮配置靠左自适应展开规则，彻底解决靠近窗口左侧边缘时 Tooltip（如 `Expand Sidebar`）被截断的问题。
- **搜索栏中英文完全对齐**：补充搜索框 `placeholder`、`aria-label` 及清空按钮的中英文国际化，消除硬编码中文。

## 0.7.7 更新

- **全面补齐界面与交互英文国际化**：彻底替换客户端界面中残留的硬编码中文，覆盖登录/注册/密码重置表单、设置面板（导出导入说明）、工作区侧边栏、笔记列表批量操作栏、冲突解决器、版本历史、标签管理、PDF 导出与分页预览、快捷键说明及全部全局通知提示。
- **编辑器与无障碍支持**：编辑器正文占位符、无障碍标签（`aria-label`）、图表渲染错误提示全面支持双语。
- **词条对齐与类型检查**：补齐 `src/client/i18n.ts` 双语字典并消除重复属性定义，通过自动化 Key 对齐校验与 TypeScript 检查。
...
- **多标签页编辑锁主动接管**：通过原生 `BroadcastChannel` 建立同源标签页通信；当在未持有锁的标签页点击「重新打开」时，主动通知持有锁的标签页释放锁并完成无缝接管，支持在多标签页之间按需自由切换。
- **按钮提示与文案对齐**：为桌面端移入回收站/恢复按钮添加显式类名并配置靠右对齐样式，彻底解决最右侧按钮 Tooltip 在窗口边缘被截断的问题；补齐锁屏状态与操作提示文案的国际化翻译。

## 0.7.4 更新

- **标签浮层交互增强**：标签选择浮层支持按 `ESC` 键关闭与点击外部区域自动收起，展开时自动聚焦搜索框；新增显式关闭按钮。
- **界面排版与国际化细节修复**：修复多标签并发锁屏界面的「重新打开」主按钮样式与英文翻译；修复顶部工具栏最右侧按钮 Tooltip（如 `Move to Trash`）在窗口边缘被截断的问题。
...
- **国际化与翻译修复**：英文模式下日期与时间统一动态使用 `en-US` 格式；新增 `data-tooltip` 与 `alt` 属性翻译支持；重排短语字典修复短词条优先冲突（如标签操作通知、修订版本提示、笔记大小超限与冲突副本后缀）；补齐动态通知与错误提示翻译。

## 0.7.1 更新

- **修复桌面端布局异常**：修复桌面端误显示移动端导航、搜索和新建悬浮按钮导致的笔记列表标题折行及多余控件问题；恢复底部字符计数样式及桌面端整洁布局。

## 0.7.0 更新

- **全新标签管理系统**：重新设计「管理标签」弹窗，清晰支持「新增」「编辑」「合并」「删除」四大操作，支持直观浏览已有标签列表。
- **优化单篇笔记打标体验**：笔记底部标签区采用清晰直观的标签胶囊（Tag Chips）展示；支持单篇打标、去除打标与新增打标，彻底杜绝在单篇笔记处误删全局标签的问题。

## 0.6.0 更新

- 标签管理支持多选，已有标签可直接选中或取消。
- UI 默认中文，可在设置中切换英文；桌面端与 320px 移动端均适配英文长度。

## 0.5.1 更新

- 已有标签选择器移至标签输入框左侧，并修复选择器文字裁切问题。
...
- AI 读写分离接入：受限令牌、精确笔记统计、跨正常/归档笔记的 FTS5 相关度搜索、命中字符范围、批量读取、MCP Resources 和写入工具；AI 修改进入正常版本历史。
...
预览中的标题、段落、列表项、表格和代码块可双击切回对应的 Markdown 源码位置。

### 标题层级与大纲

应用已提供独立的笔记标题栏，标题建议直接写文字，不添加 Markdown 的 `#`。正文不重复笔记标题：大章节从 `##` 开始，子章节使用 `###`，继续按层级递进。单独导入 Markdown/TXT 时仍可使用标准的 `# 标题` 首行，导入程序会去除 `#` 后提取为笔记标题。侧边导航面板会从正文标题实时提取层级大纲，点击即可定位。

### 待办事项

使用 `- [ ] 待办内容` 创建未完成事项，使用 `- [x] 已完成内容` 标记完成；同时兼容 `- [] 待办内容` 简写。预览模式可直接点击勾选框，状态会自动同步回写至 Markdown 源码并正常保存。
...
支持标准 GitHub Flavored Markdown 表格，并使用冒号语法定义列对齐：
...
表格在预览模式下支持双击切回 Markdown 源码行，并在桌面端和移动端自适应排版。

### 代码块与语法高亮

代码块使用带有语言标识的 fenced code block（基于 `highlight.js/lib/common`），支持常见编程语言：`typescript`, `javascript`, `python`, `bash`, `json`, `html`, `css`, `yaml`, `sql`, `go` 等；未知语言自动降级为纯文本安全显示。在预览模式中双击代码块可快速定位至编辑器源码行。
...
在预览模式下，点击上标角标（如 `[1]`）可平滑滚动至文末脚注定义；点击脚注右侧的回跳链接（`↩︎`）可无缝返回正文对应位置。

### 知识链接 (WikiLinks) 与反向链接

支持使用 `[[笔记 UUID|显示标题]]` 或 `[[笔记 UUID]]` 建立应用内笔记间的双向链接。链接以稳定的 UUID 标识，目标笔记修改标题后链接依然有效；在预览模式下点击链接可在应用内直接切换并打开目标笔记。

```markdown
参考相关文档：[[00000000-0000-0000-0000-000000000001|项目开发规范]]
```

侧栏大纲面板会自动扫描所有活跃笔记，实时列出当前笔记的反向链接（哪些笔记引用了当前笔记），构建个人双向知识网。
...
流程图和脑图使用 Mermaid fenced code block，在预览模式渲染，支持暗黑模式自动切换：
...
脑图将首行改为 `mindmap` 并按缩进编写节点，亦支持 `sequenceDiagram`、`classDiagram`、`stateDiagram` 等常用图表。单篇笔记最多渲染 20 个 Mermaid 图表，每个源码最多 50,000 字符。

HTML block 可直接写入 Markdown，通过 DOMPurify 进行安全过滤。放行 `<section>`、`<mark>`、`<kbd>`、`<details>`、`<summary>` 等安全结构标签；自动剥离 `<script>`、`<style>`、`<iframe>`、`<form>`、事件属性和外部媒体。

可直接在应用中导入 [`examples/markdown-syntax-demo.md`](examples/markdown-syntax-demo.md) 体验全套语法渲染，或导入 [`examples/mermaid-html-demo.md`](examples/mermaid-html-demo.md) 验收图表与 HTML block。

### 图片与附件

- **私有图片语法**：`![图片描述](/api/images/<uuid>)`。支持原图上传（不进行有损压缩或 EXIF 清除）、按需懒加载、离线 IndexedDB 缓存兜底，并在预览模式下点击触发全屏灯箱放大查看。
- **私有附件语法**：`[附件名称.pdf](/api/files/<uuid>)`。针对 PDF、TXT、CSV、JSON、ZIP 等格式，预览模式点击自动触发浏览器安全下载（强制附加 `download` 属性与 `nosniff` 防嗅探）。
- **外部图片防追踪拦截**：为避免访问追踪和隐私泄露，正文中的外部 HTTP(S) 图片默认不自动加载，渲染为 `[外部图片未加载]` 占位符。
- **安全与存储架构**：
